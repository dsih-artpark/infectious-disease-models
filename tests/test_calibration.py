...
def test_loss_function_runs():
    ...
    y = model.simulate(init_conditions, t)  # <-- pass dict directly
    loss = calib.loss_function([0.3, 0.1], init_conditions, t, y[:, 1], [1])
...
def test_fit_with_optimizer():
    ...
    t = np.linspace(0, 5, 6)
    y = model.simulate(init_conditions, t)  # <-- pass dict directly
    results = calib.fit(init_conditions, t, t, y[:, 1], optimizers=["Nelder-Mead"], compartments=[1])
